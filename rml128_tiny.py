    ...
    for mod in mods:
        for snr in snrs:
            ...
            for i in range(Xd[(mod,snr)].shape[0]):
                lbl.append((mod,snr))
            train_idx+=list(np.random.choice(range(a*1000,(a+1)*1000), size=10, replace=False))
            val_idx+=list(np.random.choice(list(set(range(a*1000,(a+1)*1000))-set(train_idx)), size=2, replace=False))
            if snr >7:
                test_idx+=list(np.random.choice(list(set(range(a*1000,(a+1)*1000))-set(train_idx)-set(val_idx)), size=5, replace=False))
            a+=1
