...
def load_data(filename=r'RML2016.10a_dict.pkl'):
    Xd =pickle.load(open(filename,'rb'),encoding='iso-8859-1')#Xd(120W,2,128) 10class*20SNR*6000samples
    ...
    for mod in mods:
        for snr in snrs:
            ...
            for i in range(Xd[(mod,snr)].shape[0]):
                lbl.append((mod,snr))
            train_idx+=list(np.random.choice(range(a*1000,(a+1)*1000), size=60, replace=False))
            val_idx+=list(np.random.choice(list(set(range(a*1000,(a+1)*1000))-set(train_idx)), size=20, replace=False))
            test_idx+=list(np.random.choice(list(set(range(a*1000,(a+1)*1000))-set(train_idx)-set(val_idx)), size=15, replace=False))
            a+=1

#     print(np.array(train_idx).shape)
    X = np.vstack(X)
